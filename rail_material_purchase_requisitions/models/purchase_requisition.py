...
#                raise UserError(_('You can not delete Purchase Requisition which is not in draft or cancelled or rejected state.'))
    ...
    def requisition_confirm(self):
        for rec in self:
            manager_mail_template = self.env.ref('rail_material_purchase_requisitions.email_confirm_material_purchase_requistion')
    ...
    def manager_approve(self):
        for rec in self:
            ...
            employee_mail_template = self.env.ref('rail_material_purchase_requisitions.email_purchase_requisition_iruser_custom')
            email_iruser_template = self.env.ref('rail_material_purchase_requisitions.email_purchase_requisition')
