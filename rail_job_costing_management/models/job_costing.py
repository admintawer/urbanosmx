...
class JobCosting(models.Model):
    ...
    def onchange_so_number(self):
        if self.so_number:
            self.partner_id = self.so_number.partner_id.id
    ...
    def action_view_jobcost_sheet_lines(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("rail_job_costing_management.action_job_cost_line_custom")
        action['domain'] = [('id', 'in', cost_ids)]
