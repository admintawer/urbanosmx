...
def convert_smart_date_to_string(date_str, env=None):
    if not isinstance(date_str, str) or not date_str.startswith(('today', 'now')):
        return date_str
    
    env = env or request.env
    result = parse_date(date_str, env)
...
def compute_domain(domain_tuple,model):
    ...
    for field in left_value_split_list:
        ...
        if field_type in ['many2one', 'many2many', 'one2many']:
            ...
            if relation_model == 'res.company': 
                left_company = True
        elif field_type == 'integer' and model_field._fields.get('id'):
            if model_obj == 'res.users':
                left_user = True

            if model_obj == 'res.company':
                left_company = True
    ...
    if left_user:
        if operator_value in ['in', 'not in']:
            ...
            elif isinstance(right_value, list) and -1 in right_value:
                parent_user = request.env.user

                user_ids = request.env['res.users'].search([
                    ('id', 'child_of', parent_user.id)
                ]).ids

                right_value[:] = [v for v in right_value if v != -1]
                right_value.extend(user_ids)
    ...
    if _is_date_field(left_value, model):
        if isinstance(right_value, str) and right_value.startswith(('today', 'now')):
            right_value = convert_smart_date_to_string(right_value, request.env)
        elif isinstance(right_value, list):
            right_value = [
                convert_smart_date_to_string(v, request.env) if isinstance(v, str) and v.startswith(('today', 'now')) else v 
...
def _is_date_field(field_name, model):
    field_obj = request.env['ir.model.fields'].sudo()
    field_search = field_obj.search([
        ('model_id.model', '=', request.env[model]._name),
        ('name', '=', field_name)
    ], limit=1)
    return bool(field_search and field_search.ttype in ['date', 'datetime'])
