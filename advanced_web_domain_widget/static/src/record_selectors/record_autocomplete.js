...
export class RecordAutocompleteBits extends Component {
  ...
  async loadOptionsSource(name) {
    ...
    // Add environment options for specific models
    if (this.props.resModel === "res.users") {
        options.unshift({
            data: {
                record: { id: 0, display_name: 'Environment User' },
                img: "/advanced_web_domain_widget/static/description/img/env_user.png",
            },
            label: 'Environment User',
            onSelect: () => this.props.update([0]),
        });
    }
    if (this.props.resModel === "res.company") {
        options.unshift({
            data: {
                record: { id: 0, display_name: 'Environment Companies' },
            },
            label: 'Environment Companies',
            onSelect: () => this.props.update([0]),
        });
    }
    ...
    async onSearchMore(name) {
        ...
        if (name) {
            ...
            if (this.props.resModel === "res.company") {
                nameGets.unshift([0, "Environment Companies"]);
            }
        } else {
            operator = "not in";
